...
void add_corrected(gauss_t *z, const gauss_t x, const gauss_t y) {
	set_gauss(z, add_int32_t(x.re,y.re), add_int32_t(x.img,y.img));
}

void add(gauss_t *z, const gauss_t x, const gauss_t y) {
	set_gauss(z, x.re + y.re, x.img + y.img);
}

void sub_corrected(gauss_t *z, const gauss_t x, const gauss_t y) {
