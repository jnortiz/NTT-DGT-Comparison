...
void poly_dgt(poly *r)
{
  ...
  for(i = 0; i < N; i += 2) 
  {
    ...
    c = (uint64_t)copy[i] * nthroots[i+1];
    d = (uint64_t)copy[i+1] * nthroots[i];
...
void poly_invdgt_montgomery(poly *r)
{
  ...
  for(i = 0; i < N; i += 2) 
  {
    ...
    c = (uint64_t)copy[i] * invnthroots[i+1];
    d = (uint64_t)copy[i+1] * invnthroots[i];
...
void poly_pointwise_invmontgomery(poly *r, const poly *a, const poly *b)
{
  ...
  for(i = 0; i < N; i += 2) 
  {
    r->coeffs[i] = montgomery_reduce((uint64_t)a->coeffs[i] * b->coeffs[i]) +
                    (2*Q - montgomery_reduce((uint64_t)a->coeffs[i+1] * b->coeffs[i+1]));

    r->coeffs[i+1] = montgomery_reduce(
		     (uint64_t)a->coeffs[i] * b->coeffs[i+1] + 
                     (uint64_t)a->coeffs[i+1] * b->coeffs[i]);
  }
