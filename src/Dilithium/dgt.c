...
{ 
  ...
  for(; m > 2; m >>= 1) 
  {
    ...
    for(j = 0; j < m; j += 2) 
    {
      ...
      for(i = j; i < N; i += (m << 1)) 
      {
        sub_re = copy[i] + (2*Q - copy[i+m]);
        sub_img = copy[i+1] + (2*Q - copy[i+m+1]);
        
        copy[i] = reduce32(copy[i] + copy[i+m]);
        copy[i+1] = reduce32(copy[i+1] + copy[i+m+1]);
        
        copy[i+m] = montgomery_reduce(a * sub_re);
        copy[i+m+1] = montgomery_reduce(a * sub_img);        
      }
      index += window;
    }
    window <<= 1;
...
void idgt(uint32_t *poly)
{
  ...
  for(m = 2; m < N; m <<= 1) 
  {
    ...
    for(j = 0; j < m; j += 2) 
    {
      ...
      for(i = j; i < N; i += (m << 1)) 
      {
        mul_re = montgomery_reduce(copy[i+m] * a);
        mul_img = montgomery_reduce(copy[i+m+1] * a);
        
        copy[i+m] = copy[i] + (2*Q - mul_re);
        copy[i+m+1] = copy[i+1] + (2*Q - mul_img);
