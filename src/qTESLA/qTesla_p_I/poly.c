...
void dgt(poly x)
{
  ...
  for(; m >= 4; m >>= 1) {
    ...
    for(j = 0; j < m; j += 2) {
      ...
      sub_re = (x[i] + (2LL*PARAM_Q - x[i+m]));
      sub_img = (x[i+1] + (2LL*PARAM_Q -x[i+m+1]));
    ...
    for(j = 0; j < m; j += 2) {
      ...
      x[i] = barr_reduce64(x[i] + x[i+m]);
      x[i+1] = barr_reduce64(x[i+1] + x[i+m+1]);
  ...
  for(m = 2; m <= 256; m <<= 1) {
    ...
    for(j = 0; j < m; j += 2) {
      ...
      for(i = j; i < PARAM_N; i += (m << 1)) {
        ...
        x[i+m] = (x[i] + (2LL*PARAM_Q - mul_re));
        x[i+m+1] = (x[i+1] + (2LL*PARAM_Q - mul_img));
    ...
    for(j = 0; j < m; j += 2) {
      ...
      for(i = j; i < PARAM_N; i += (m << 1)) {
        ...
        x[i+m] = barr_reduce64(x[i] + (2LL*PARAM_Q - mul_re));
        x[i+m+1] = barr_reduce64(x[i+1] + (2LL*PARAM_Q - mul_img));

        x[i] = barr_reduce64(x[i] + mul_re);
        x[i+1] = barr_reduce64(x[i+1] + mul_img);        
      }
      index += window;
    }
    window >>= 1;
