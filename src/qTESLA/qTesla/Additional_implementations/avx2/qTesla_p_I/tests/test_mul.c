...
int main(void) {
  ...
  for(i = 0; i < NRUNS; ++i) {
    ...
    cycles4[i] = cpucycles();
    poly_ntt_asm(a_ntt, a, zeta);  
    cycles4[i] = cpucycles() - cycles4[i];

    cycles5[i] = cpucycles();
    poly_intt_asm(a_ntt, a, zetainv);  
    cycles5[i] = cpucycles() - cycles5[i];  
}

  print_results("naive: ", cycles0, NRUNS);
  print_results("ntt_mul: ", cycles1, NRUNS);
  print_results("poly_ntt: ", cycles2, NRUNS);
  print_results("poly_mul: ", cycles3, NRUNS);
  print_results("ntt_asm: ", cycles4, NRUNS);
  print_results("intt_asm: ", cycles5, NRUNS);
