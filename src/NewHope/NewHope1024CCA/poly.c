...
void poly_mul(poly *r, const poly *a, const poly *b)
{
  ...
  for(i = 0; i < NEWHOPE_N; i += 2) 
  {
    ...
    r->coeffs[i+1] = montgomery_reduce(
		    (uint32_t)t * b->coeffs[i+1] + 
                    (uint32_t)s * b->coeffs[i]);
  }  
}
...
void poly_dgt(poly *r)
{
  ...
  // Folding the input vector into K/2 Gaussian integers
  j = 0;
  for(i = 0; i < 512; ++i)
  {
    r->coeffs[j] = copy[i];
    r->coeffs[j+1] = copy[i+512];
...
void poly_invdgt(poly *r)
{
  ...
  // Unfolding the output vector
  j = 0;
  for(i = 0; i < NEWHOPE_N; i += 2)
  {
    r->coeffs[j] = copy[i];
    r->coeffs[j+512] = copy[i+1];
    j++;
  }
}
