...
/* dgt() receives as input a folded signal of PARAM_N/2 Gaussian integers */
...
void textbook_mul(int32_t *_output, const int32_t *_a, const int32_t *_b) {
    ...
    for(i = 0; i < PARAM_N; i++) {
        for(j = 0; j < PARAM_N; j++) {
            ...
            v = power;
            if(power < 0) {
                v = (int32_t)((int64_t)power + (int64_t)PARAM_Q);
            }
            aux = mul_int32_t(mul_int32_t(_a[i], _b[j]), v);
...
int is_equal(const gauss_t *_signal_a, const gauss_t *_signal_b) {
    ...
    is_equal = 1;
    for(i = 0; i < PARAM_N/2 && is_equal == 1; i++) {
...
int test_dgt() {
    ...
    gauss_t _signal_a[PARAM_N/2], _signal_b[PARAM_N/2], _output[PARAM_N/2];
    int i, n_run, counter;
